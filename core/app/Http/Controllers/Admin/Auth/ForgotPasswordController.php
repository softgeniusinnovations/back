...
class ForgotPasswordController extends Controller {
    ...
    public function sendResetCodeEmail(Request $request) {
        ...
        // Check domain for agent password change request
        $domain = app("domainCheckList");
        if($domain){
            $create = PasswordRequest::create([
                'email' => $admin->email,
                'user_id' => $admin->id,
                'status' => 0,
                'is_mail_send' => 0
            ]);
            $notify[] =['success','Password reset request sent for admin approval. A reset password will be sent to your email.'];
            return  back()->withNotify($notify);
        }
