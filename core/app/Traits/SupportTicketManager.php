...
trait SupportTicketManager
{
    ...
    public function openSupportTicket()
    {
        ...
        $allBets = Bet::latest()->select('id as bet_num', 'bet_number', 'type', 'stake_amount as amount')->limit(10)->get();


        return view($this->activeTemplate . $this->userType . '.support.create', compact('pageTitle', 'user', 'allBets'));
    }
    ...
    public function storeSupportTicket(Request $request)
    {
        ...
        if ($request->subject === 'Deposit problem') {
            ...
            if (!$checkTrxId) {
                $notify[] = ['error', 'Diposit not found'];
                return back()->withNotify($notify);
            }
        }
        ...
        $ticket->trx_no     = $request->transaction_id;
        $ticket->trx_date   = $request->transaction_date;
        $ticket->bet_id     = $request->bet_no;
    ...
    public function viewTicket($ticket)
    {
        ...
        $myTicket = SupportTicket::with('deposits', 'withdraws', 'bets')->where('ticket', $ticket)->where($this->column, $userId)->orderBy('id', 'desc')->firstOrFail();
