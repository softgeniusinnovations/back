                    ...
                    <h6 class="card-title  mb-4">
                        <div class="row">
                            <div class="col-sm-8 col-md-6">
                                ...
                                [@lang('Ticket#'){{ $ticket->ticket }}] {{ $ticket->subject }} <br> <br>
                                @if ($ticket->subject === 'Deposit problem')
                                    <a
                                        href="{{ route('admin.deposit.details', $ticket->deposits->id) }}">{{ $ticket->trx_no }}</a>
                                    <br />
                                    <a
                                        href="{{ route('admin.agent.details', @$ticket->deposits->agent->id) }}">{{ '@Agent-' . @$ticket->deposits->agent->name }}</a>
                                @endif
                                @if ($ticket->subject === 'Withdraw problem')
                                    <a
                                        href="{{ route('admin.withdraw.details', $ticket->withdraws->id) }}">{{ $ticket->trx_no }}</a>
                                    <br />
                                    <a
                                        href="{{ route('admin.agent.details', @$ticket->withdraws->agent->id) }}">{{ '@Agent-' . @$ticket->withdraws->agent->name }}</a>
                                @endif
                                @if ($ticket->subject === 'Bet issue')
                                    {{ @$ticket->bets->bet_number ? 'Bet: ' . @$ticket->bets->bet_number : '' }}
                                    {{ @$ticket->bets->stake_amount ? 'Amount: ' . @$ticket->bets->stake_amount : '' }}
                                    <br />
                                    {{ @$ticket->bets->type ? (@$ticket->bets->type == 1 ? 'Single' : 'Multiple') : '' }}
                                    <br />
                                    @php
                                        $BetStatus = ['', 'Win', 'Pending', 'Lose', 'Refunded'];
                                    @endphp
                                    {{ @$ticket->bets->status ? 'Status:' . $BetStatus[@$ticket->bets->status] : '' }}
                                @endif
                            </div>
