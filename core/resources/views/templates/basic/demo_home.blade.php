...
@section('bet')
    ...
    <div class="col-12">
        <div class="betting-body">
            ...
            <div class="row g-3 game-sliders owl-carousel owl-theme" >
                @php
                    $user = auth()->user();
                @endphp
                @foreach ($liveGames as $game)
                    <div class="col-sm-6 col-lg-6 col-md-4 col-xl-4 col-xxl-3 col-msm-6 item" style=" width: 100%;height: 100%;">
                                ...
                                <div class="option-odd-list">
                                    @foreach ($firstMarket->options as $option)
                                        <div class="option-odd-list__item just-open-betslip">
                                            ...
                                            <div>
                                                {{-- <button class="btn btn-sm btn-light text--small border oddBtn @if (in_array($option->id, $optionsId)) active @endif @if ($option->locked) locked @endif" data-option_id="{{ $option->id }}" @disabled($game->bet_start_time >= now())>{{ rateData($option->odds) }} </button> --}}
                                        <button class="btn btn-sm btn-light text--small border oddBtn @if (in_array($option->id, $optionsId)) active @endif @if ($option->locked) locked @endif" data-option_id="{{ $option->id }}" >{{ rateData($option->odds) }} </button>
                                                {{-- @auth
                                                <button class="btn btn-sm btn-light text--small border oddBtn @if (in_array($option->id, $optionsId)) active @endif @if ($option->locked) locked @endif" data-option_id="{{ $option->id }}" @disabled($game->bet_start_time >= now())>{{ rateData($option->odds) }} </button>
                                                @endauth
                                                @guest
